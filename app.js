...
function renderChart(container, hourly) {
  ...
  // 画布（viewBox 固定逻辑尺寸，CSS 自适应宽度）
  const W = 960, H = 300, mL = 48, mR = 48, mT = 14, mB = 34;
  const pw = W - mL - mR, ph = H - mT - mB;
  const X = (k) => mL + (k / (idx.length - 1)) * pw;
  const Yt = (v) => mT + ph - ((v - tMin) / (tMax - tMin)) * ph;
  const Yp = (v) => mT + ph - (v / 100) * ph;
  ...
  // 折线序列定义
  const series = [
    { name: '气温',       color: '#ef4444', width: 2.5, dash: '',    get: i => temp[i],  y: Yt, unit: '°C', digits: 1 },
    { name: '降水概率',   color: '#2563eb', width: 1.8, dash: '',    get: i => precip[i], y: Yp, unit: '%',  digits: 0 },
    { name: '低云',       color: '#d97706', width: 1.5, dash: '4 3', get: i => ccLow[i],  y: Yp, unit: '%',  digits: 0 },
    { name: '中云',       color: '#7c3aed', width: 1.5, dash: '4 3', get: i => ccMid[i],  y: Yp, unit: '%',  digits: 0 },
    { name: '高云',       color: '#64748b', width: 1.5, dash: '2 3', get: i => ccHigh[i], y: Yp, unit: '%',  digits: 0 },
  ];

  // 折线 path（跳过 null / 非数值点，断线处理）
  const pathFor = (s) => {
    let d = '', pen = false;
    for (let k = 0; k < idx.length; k++) {
      const v = s.get(idx[k]);
      if (v === null || v === undefined || !Number.isFinite(v)) { pen = false; continue; }
      d += (pen ? ' L' : ' M') + X(k).toFixed(1) + ' ' + s.y(v).toFixed(1);
      pen = true;
    }
    return d;
  };
  ...
  for (const v of pTicks) {
    ...
    inner += `<line x1="${mL}" y1="${y}" x2="${W - mR}" y2="${y}" class="grid-line"/>`;
    inner += `<text x="${W - mR + 6}" y="${(Number(y) + 4).toFixed(1)}" class="axis-text">${v}%</text>`;
  }
  // 左轴（温度刻度文本）
  for (const v of tTicks) {
    inner += `<text x="${mL - 8}" y="${(Yt(v) + 4).toFixed(1)}" class="axis-text" text-anchor="end">${v}°</text>`;
  }
  // 轴标签
  inner += `<text x="${mL - 18}" y="${mT + 4}" class="axis-text" text-anchor="middle">°C</text>`;
  inner += `<text x="${W - mR + 18}" y="${mT + 4}" class="axis-text" text-anchor="middle">%</text>`;

  // 每天 00:00 分隔虚线
  for (let k = 0; k < idx.length; k++) {
    if (idx[k] % 24 === 0) {
      const x = X(k).toFixed(1);
      inner += `<line x1="${x}" y1="${mT}" x2="${x}" y2="${H - mB}" class="day-line"/>`;
    }
  }
  ...
  for (const s of series) {
    const d = pathFor(s);
    if (d) {
      inner += `<path d="${d}" fill="none" stroke="${s.color}" stroke-width="${s.width}" ` +
  ...
  for (let k = 0; k < idx.length; k += labelEvery) {
    inner += `<text x="${X(k).toFixed(1)}" y="${H - mB + 18}" class="axis-text" text-anchor="middle">${fmtTime(time[idx[k]])}</text>`;
  }

  // hover 热区（透明矩形）
  const hotW = pw / (idx.length - 1);
  for (let k = 0; k < idx.length; k++) {
    inner += `<rect x="${(X(k) - hotW / 2).toFixed(1)}" y="${mT}" width="${hotW.toFixed(1)}" height="${ph}" class="hot" data-k="${k}"/>`;
  }
  ...
  legend.innerHTML = series.map(s =>
    `<span class="lg-item"><i style="background:${s.color}"></i>${s.name}</span>`).join('');
  ...
  svg.setAttribute('aria-label', '逐小时气温、降水概率与云量趋势图');
  svg.innerHTML = inner;

  const tip = document.createElement('div');
  tip.className = 'chart-tip';

  container.append(legend, svg, tip);
